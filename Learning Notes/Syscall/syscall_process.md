...
Syscall in user process looks like a normal function. (i.e. fork())

It may also named `fork()` in kernel. However user cannot directly invoke it, due to different executing environmant and priviledge level. 

Therefore before truely calling the kernel function i.e. fork() in kernel, we must complete a series of work, which allow the CPU switch from user mode to kernel mode

![syscall](./syscall.png)

## How does System Call implemented in User and Kernel side in xv6 ?

### User side: 
`user/usys.S` `include/syscall.h` `include/user.h`
...
The kernel use syscall number to determine which kernel function to invoke when it receive syscall interrupt
...
#### include/user.h
user.h provides the function prototypes of the system calls, allowing the compiler to perform type checking and generate function call instructions for those symbols when compiling user programs.

During the linking stage, the linker resolves these function calls to the corresponding system call wrapper functions defined in usys.S.




### Kernel Side : 
`kernel/vector.pl` `kernel/syscall.c` `kernel/trapasm.S` `kernel/trap.c` 
...
This is because for interrupts like page fault, Invalid TSS, the CPU will push an extra errno to the stack. But we want a consistent trapframe. Thus for other function, we just push a fake errno number.
...
As you observe from all the vector function: Every interrupt functions will jump to `alltrap`(defined in `trapasm.S`):
...
`void trap(struct trapframe *tf)` check if the current process that made the system call is killed or not; If so then just simply exit and cleans up the process. (Not proceeeding the syscall). Otherwise call the `syscall()` to perform the actual system call. Finally, it checks whether the process has been killed again before returning.


Here according to the trapno, we invoke the respective syscall defined in `sysproc.c` and `proc.c`. When the syscall returns, we need to 1. 

1. Pop back the segment registers. 

2. Pop back to resume CPU state.  

3. Clear the trapno and errcode 
